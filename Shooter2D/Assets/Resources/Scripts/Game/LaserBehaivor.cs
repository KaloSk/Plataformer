...
public class LaserBehaivor : MonoBehaviour {
	...
	void Update () {
        ...
        if (hit.collider != null && hit.collider.tag != "Untagged")
        {
            ...
            if (hit.collider.CompareTag("SimpleEnemy") || hit.collider.CompareTag("PlayerBulletWall"))
            {
        ...
        // Place things
        // -- Gather some data
        float startSpriteWidth = start.GetComponent<SpriteRenderer>().bounds.size.y;
        float endSpriteWidth = 0f;
        if (end != null) endSpriteWidth = end.GetComponent<SpriteRenderer>().bounds.size.y;
